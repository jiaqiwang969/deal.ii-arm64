...
Poisson<dim>::output_results(const unsigned cycle) const
{
  ...
  // data_out.add_data_vector(locally_interpolated_exact, "exact");
...
Poisson<dim>::print_system_info()
{
  ...
  pcout << "Number of cores  : " << MultithreadInfo::n_cores() << std::endl
        << "Number of threads: " << MultithreadInfo::n_threads() << std::endl;
}
